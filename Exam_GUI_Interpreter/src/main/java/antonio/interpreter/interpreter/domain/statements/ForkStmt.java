...
public class ForkStmt implements IStmt {
    ...
    public PrgState execute(PrgState state) throws StmtException, ExpException, MyDictionaryException {
        return new PrgState(new ExeStack(), state.getSymbolsTable().deepCopy(), state.getOutputList(), state.getFileTable(), state.getHeap(), state.getLatchTable(), this.innerStmt);
    }
